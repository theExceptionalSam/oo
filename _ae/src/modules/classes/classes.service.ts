...
export class ClassesService {
  ...
  async create(payload: Partial<Class>, caller?: Caller): Promise<Class> {
    if (caller && !isSuperAdmin(caller) && caller.school_id) {
      payload = { ...payload, schoolId: caller.school_id };
    }
    const entity = this.repo.create(payload);
    const saved = await this.repo.save(entity);
    this.logger.log(`Created Class id=${saved.id}`);
    return saved;
  }
  ...
  } = {}, caller?: Caller): Promise<{ items: Class[]; meta: { page: number; limit: number; total: number } }> {
  ...
  async findOne(id: string, caller?: Caller): Promise<Class> {
  ...
  async update(id: string, payload: Partial<Class>, caller?: Caller): Promise<Class> {
