...
export class SubjectsService {
  ...
  async create(payload: Partial<Subject>, caller?: Caller): Promise<Subject> {
    if (caller && !isSuperAdmin(caller) && caller.school_id) {
      payload = { ...payload, schoolId: caller.school_id };
    }
    const entity = this.repo.create(payload);
    const saved = await this.repo.save(entity);
    this.logger.log(`Created Subject id=${saved.id}`);
    return saved;
  }
  ...
  } = {}, caller?: Caller): Promise<{ items: Subject[]; meta: { page: number; limit: number; total: number } }> {
  ...
  async findOne(id: string, caller?: Caller): Promise<Subject> {
    const entity = await this.repo.findOne({ where: { id } as never });
    if (!entity) throw new NotFoundException(`Subject ${id} not found`);
    assertSameTenant(caller, (entity as unknown as { schoolId?: string }).schoolId);
    return entity;
  }

  async update(id: string, payload: Partial<Subject>, caller?: Caller): Promise<Subject> {
