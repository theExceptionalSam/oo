...
export class AcademicYearsService {
  ...
  async create(payload: Partial<AcademicYear>, caller?: Caller): Promise<AcademicYear> {
    if (caller && !isSuperAdmin(caller) && caller.school_id) {
      payload = { ...payload, schoolId: caller.school_id };
    }
    const entity = this.repo.create(payload);
    const saved = await this.repo.save(entity);
    this.logger.log(`Created AcademicYear id=${saved.id}`);
    return saved;
  }
  ...
  } = {}, caller?: Caller): Promise<{ items: AcademicYear[]; meta: { page: number; limit: number; total: number } }> {
  ...
  async findOne(id: string, caller?: Caller): Promise<AcademicYear> {
    const entity = await this.repo.findOne({ where: { id } as never });
    if (!entity) throw new NotFoundException(`AcademicYear ${id} not found`);
    assertSameTenant(caller, (entity as unknown as { schoolId?: string }).schoolId);
    return entity;
  }

  async update(id: string, payload: Partial<AcademicYear>, caller?: Caller): Promise<AcademicYear> {
